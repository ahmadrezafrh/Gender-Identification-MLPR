...
def main():
    ...
    if len(configue['c_values']) != 1:
    	...
    	cfg.add(key="c_values",value=[10**int(configue['c_values'][0])])
    ...
    print("Start running RBF models.")
    for param in params:
        
        print('model type: RBF SVM')
        print(f'c: {param["c_values"]}')
        print(f'gamma: {param["gammas"]}')
        ...
        if param["splits"]=="k-folds":
            ...
            for fold in folds:
                ...
                if param["balance_priors"][0] == 'balanced':                            
                    model.fit(x_train, y_train, gamma=param["gammas"], c=param["c_values"], balance=True, pi=param["balance_priors"][1])
                else:
                    model.fit(x_train, y_train, gamma=param["gammas"], c=param["c_values"])
    ...
    print("End running rbf SVM models.\n")
